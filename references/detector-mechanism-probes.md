...
5. 只有 `LITERARY_REGRESSION=PASS` 且用户在同一检测条件下明确登记 `USER_DETECTOR_ACCEPTANCE=PASS`，候选才可进入项目后续门；该状态是项目验收结果，不是 Skill 对其他检测器的保证。

选定回归对象 后续回归固定使用 `DETECTOR_SUBMISSION_PROFILE=BODY_ONLY_UTF8_LF_V1`：源稿只含无标题正文主体、UTF-8、LF 换行、无 BOM；标题版只可作为 P2A 影子对照。若用户实际使用粘贴文本或其他容器，必须另登记`SUBMISSION_SHA256`及`DETECTOR_SUBMISSION_NORMALIZATION`，不能假设原文件字节原样进入检测器。只要归一化转换与报告字符数机械闭合，可在同一报告内建立源稿到提交文本的映射；跨轮比较仍须保持同一提交方式和转换。
...
反例也必须保留：错标标点组、混母文长度梯均保持无效；分段／前缀／阈值交互不能推导普适写作规则，句群节奏和视角的单场景信号不能升级为全局规则。
