...
name: human-writing
description: 通用中文创作与改稿原则，覆盖知乎回答、论坛长帖、公众号文章、博客、评论、人物故事、历史叙事、新闻与行业解读、科普、教程、评测、个人叙事、小说、故事、对白、口播和演讲稿。默认保护说话位置、材料边界、人物行动与自然中文韵律，避免空泛机构腔、营销腔和模型腔。严格 house-style 规则只有任务或项目合同明确启用时才是硬门；不创建作者画像、个人规则库或个人写作 Skill。
---

# 活人感写作 1.1.0

> 兼容说明：本文件同时保留通用正向写作原则和一个较严格的 house-style 检查段。只有当前任务、项目合同或用户明确选择该 house-style 时，才把后段禁用项当作硬门；普通小说、人物对白、其他文体和用户自定义声线默认将其作为可解释提醒。不要让一套全局禁词把不同人物、场景或时代压成同一种声音。
...
- 新写或大幅重写知乎回答、论坛长帖、公众号文章、博客、评论、人物稿和行业稿，读取 `references/human-forum-prose.md`。
- 真人、历史、新闻、产品、数据、评测、教程、商业信息和用户亲历，另读 `references/human-reality.md`。
- 小说、故事、虚构散文、对白和剧本，改读 `references/human-fiction.md`。用户要求帖子体小说或第一人称故事时，再同时读取 `references/human-forum-prose.md`。
- 短内容、个人叙事、教程、评测、口播、演讲、剧本、对白和诗歌等形式需要特殊处理时，读取 `references/human-formats.md`。
- 初稿完成后再读取 `references/human-revision.md`。不要在动笔前加载详细审稿规则。

不要为了保险一次加载全部参考文件，只读当前任务确实命中的部分。真正的混合任务可以读取多个文件，`references/human-revision.md` 仍留到初稿以后。创作时以正向目标为主，已经写在本文件里的交付禁令留到初稿完成后再逐项检查。
...
3. `references/human-fiction.md`、`references/human-reality.md` 和 `references/human-formats.md` 中与当前任务直接相关的规则。
4. `references/human-forum-prose.md` 与本文件里的默认散文写法。
5. `references/human-revision.md` 的改稿规则与检查脚本的提醒。
...
现实稿的第三项太少，回到开稿前的材料检查。不要用哲理、象征、想象现场和重复结论补字数。六千字现实长文需要能展开多个阶段、案例或现实关系。虚构稿若缺少人物目标、行动和场景变化，回到 `references/human-fiction.md` 补出故事，不向用户索要不存在的现实出处。
...
## 可选严格 house-style：成稿绝对不能出现

以下规则只在 `STYLE_PROFILE=STRICT_HOUSE_STYLE` 或项目交接明确声明启用时生效。未选择时，脚本命中只生成 `REVIEW_FLAG`，必须结合当前文体、人物声口和语境判断；不可为了清零命中而统一改句。
...
- 不用商业汇报和模型惯用黑话替普通事情抬价。`references/human-revision.md` 区分绝对禁词和需要结合语境判断的词，检查脚本不得自行增加未写明的硬禁词。
...
长稿可以保存为 Markdown 或文本后运行 `scripts/check_human_writing.py <稿件路径>`。脚本失败就继续修改，直到禁用项清零。

## 交稿

用户只要作品，就只交作品。不要展示内部提纲、规则检查和创作过程。`references/human-reality.md` 要求的开头检索痕迹是正文的一部分，不算创作过程。
