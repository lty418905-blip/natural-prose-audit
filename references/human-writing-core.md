---
name: human-writing
description: 通用中文创作与改稿原则，覆盖回答、长帖、现实题材、小说、对白、口播和特殊格式；默认保护说话位置、材料边界、人物行动与自然中文韵律。文件末段另列一个可选严格 house-style 配置，只有任务明确选择时才执行；不创建作者画像、个人规则库或个人写作 Skill。
---

# 活人感写作 1.1.0

> 兼容说明：本文件同时保留通用正向写作原则和一个较严格的 house-style 检查段。只有用户明确选择该 house-style，或任务明确要求执行对应禁用项时，才把那一段当作硬门；普通中文创作、其他文体和用户自定义风格以 `SKILL.md`、当前文体参考及用户合同为准。脚本命中默认是可解释提醒，不是通用文学禁令。
...
不要为了保险一次加载全部参考文件，只读当前任务确实命中的部分。真正的混合任务可以读取多个文件，`references/human-revision.md` 仍留到初稿以后。创作时以正向目标为主，已经写在本文件里的可选严格项留到初稿完成后再逐项检查。
...
以下规则属于可选的严格配置，服务于需要这一套具体禁用项的任务。未选择该配置时不要把它们自动移植到其他文体或用户的既有风格。
...
长稿可以保存为 Markdown 或文本后运行 `scripts/check_human_writing.py <稿件路径>` 与 `scripts/audit_prose.py <稿件路径> --mode fiction`。脚本只报告形状；是否处理每项命中要回到当前文体和上下文。
