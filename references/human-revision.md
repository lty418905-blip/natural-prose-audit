...
逐句遮住状语和补语再读一遍。若删去以后，时间、地点、动作方向或结果、持续与数量、必要程度、因果条件、视角证据、人物声口和后续状态都没有变化，这个成分没有承担叙事功能，删掉。

不要把“轻轻、缓缓、微微、静静、默默”之类词本身列为禁词。人物压低声音会不会被人听见、动作放慢是否改变结果、程度是否决定下一步、补语是否交代方向和完成状态，都会让它们成为必要信息。相反，相邻同义修饰叠放、修饰词重复动词已经表达的意思、只给普通动作加柔光，或每隔几句用同一修饰制造精致感，均应删除或改为可观察动作与后效。

先删，不要立刻寻找另一个漂亮近义词。删后若确有信息损失，再补回最短、最具体的时间、方向、结果、程度依据或人物口吻。完整测试见`modifier-function-audit.md`。
...
这一遍检查现代散文。诗歌分行、人物对白和剧本动作可以使用省略、断句与不完整句，按 `references/human-formats.md` 和 `references/human-fiction.md` 判断。
...
本节的标点清零规则只在当前任务或项目合同明确启用严格 house-style 时生效。普通小说、人物对白和自定义声线不得把破折号、连接号式破折号或提示性冒号自动清零；没有严格合同的任务只登记位置和声线影响。
...
检查器默认只把冒号、破折号、翻案句和模型路标列为 house-style 提醒；只有显式传入 `--strict-house-style` 才将这些项目判为失败。硬停词和绝对禁用的黑话仍是硬失败。

长稿保存为文本或 Markdown 后运行下面的命令。

```text
python3 scripts/check_human_writing.py 稿件.md
```
